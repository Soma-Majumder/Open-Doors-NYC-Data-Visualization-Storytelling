...
- `data/open-doors-data.json` — the single data file the site reads. Generated by `scripts/fetch-data.mjs`. Currently seeded with real, verified aggregate figures (citywide, boroughs, charter/district, trend, bands, benchmark gap) pulled directly from the live endpoint during development, plus 2 verified real schools in the `schools` directory.
...
The very first `npm run fetch-data` run (with real internet access) will replace the 2-school seed in `data/open-doors-data.json` with the complete, live 633-school directory and re-verify every aggregate figure from scratch.

## Open items / not yet built

- The `schools` array in `data/open-doors-data.json` currently has only 2 verified real records (used for the case-study section) — the dev environment that built this project has no outbound internet access, so `scripts/fetch-data.mjs` could be written and validated (syntax-checked, dry-run tested) but never actually executed here. Running it once on a machine with real network access — or letting the GitHub Action run — populates the full directory.
...
- Consider adding a `NYC_OPEN_DATA_APP_TOKEN` repo secret (free from a Socrata account) to move the scheduled Action off the shared anonymous rate limit — not required, just a safety margin
